...
thread_local int p5_near_error = -1;
thread_local int p5_near_allocator_error = -1;
...
std::expected<void, InlineHook::Error> InlineHook::setup(
    ...
    if (auto e9_result = e9_hook(allocator); !e9_result) {
        p5_near_error = e9_result.error().type;
        p5_near_allocator_error = p5_near_error == Error::BAD_ALLOCATION
            ? static_cast<int>(e9_result.error().allocator_error) : -1;
#if SAFETYHOOK_ARCH_X86_64
