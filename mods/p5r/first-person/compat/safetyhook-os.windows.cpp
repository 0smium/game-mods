// SafetyHook (Boost Software License 1.0), P5R-local adaptation. See ../licenses/SafetyHook-BSL-1.0.txt.
#include <map>
#include <memory>
#include <mutex>
...
thread_local unsigned long p5_virtual_alloc_error = 0;
std::expected<uint8_t*, OsError> vm_allocate(uint8_t* address, size_t size, VmAccess access) {
    ...
    if (result == nullptr) {
        p5_virtual_alloc_error = GetLastError();
        return std::unexpected{OsError::FAILED_TO_ALLOCATE};
    }
...
std::expected<VmBasicInfo, OsError> vm_query(uint8_t* address) {
    ...
    VmBasicInfo info{};
    // The allocator walks VirtualQuery REGIONS, including MEM_FREE where AllocationBase is null.
    info.address = static_cast<uint8_t*>(mbi.BaseAddress);
